    ...
    function SlickGrid(container,data,columns,options) {
        ...
        function init() {
            ...
            if ($.browser.msie && $.browser.version == '6.0'){
              $headers = $("<div class='slick-header-columns' style='width:100000px; left:-10005px; top:-5px; ' />").appendTo($headerScroller);
            }
            else {
        ...
        function getMaxSupportedCssHeight() {
            ...
            var div = $("<div style='display:none' />").appendTo(document.body), currentHeight;

            while (supportedHeight <= testUpTo) {
                div.css("height", supportedHeight + increment);
                currentHeight = div.height();
                if (currentHeight !== supportedHeight + increment) {
                    supportedHeight = currentHeight > supportedHeight ? currentHeight : supportedHeight; // set the supported height to the max div height!
                    break; // stop testing the max height
                } else {
            ...
            div.remove();
            if (supportedHeight === 0)
            {
              supportedHeight = 1000000;
            }
        ...
        function destroy() {
            ...
            for (var i = 0; i < plugins.length; i++) {
                // Fix in Slickgrid code so that registered plugins are correctly unregistered again
